...
def main(templateFilename = '../json/Templates/BasicQuestions_MasterTemplate.json', useLocalModel = False, encode = False, truncateLoad = False, loadSnowflake = False, runQueries = False, saveStageFiles = False, production = False):
    ...
    if(truncateLoad):
        ak = open('../csv/answerKey.csv', 'w')
        ak.write(str(datetime.datetime.now())+'\n')
        optionSummary = open('../csv/optionSummary.csv', 'w')
        optionSummary.write('Option ID|Option Template|Combo Count|Question Count|Base Model %|Local Model %\n')
    else:
        lines = []
        if(os.path.isfile('../csv/answerKey.csv')):
            ak = open('../csv/answerKey.csv', 'r')
            ak.readline()
            lines = ak.readlines()
            ak.close()
        ak = open('../csv/answerKey.csv', 'w')
        ak.write(str(datetime.datetime.now())+'\n')
        ak.writelines(lines)
        optionSummary = open('../csv/optionSummary.csv', 'a')
    ...
    stageD = None
    if(loadSnowflake):
        stageQ = open('../csv/toStageQuery.csv', 'w')
        stageQ.write('QUESTIONID|DESC|BATCH|DASHBOARD|QUERY|ENCODING|ENCODING_JSON|RESULT_CACHE|RESULT_CACHE_TS\n')
        stageD = open('../csv/toStageDashboard.csv', 'w')
        stageD.write('QUESTIONID|DESC|BATCH|DASHBOARD|URL|ENCODING|ENCODING_JSON|FILTER|QUERY\n')
    ...
    for template in templates:
        ...
        while not done: # for each parameter combination
            ...
            if(loadSnowflake):
                ...
                stageQ.write('%s|%s|%s|%s|%s|%s|%s|%s|%s\n' % (questionID,newDesc, batch, template['urlpage'], newQuery, enc_bin, enc_json, query_cache, query_cache_ts))
                #DESC, BATCH, DASHBOARD, URL, ENCODING, ENCODING_JSON, FILTER, QUERY
                stageD.write('%s|%s|%s|%s|%s|%s|%s|%s|%s\n' % (questionID,newDesc, batch, template['urlpage'], newURL, enc_bin, enc_json, newURLFilter, newURLQuery))
    ...
    if loadSnowflake:
        ...
        session.sql('COPY INTO %s.%s (QUESTIONID, DESC, BATCH, DASHBOARD, URL, ENCODING, ENCODING_JSON, FILTER, QUERY) FROM %s file_format = (type = \'CSV\' SKIP_HEADER = 1 FIELD_DELIMITER = \'|\');' % (schema, tableDashboard, stageDashboard)).collect()
        session.sql('COPY INTO %s.%s (QUESTIONID, DESC, BATCH, DASHBOARD, QUERY, ENCODING, ENCODING_JSON, RESULT_CACHE, RESULT_CACHE_TS) FROM %s file_format = (type = \'CSV\' SKIP_HEADER = 1 FIELD_DELIMITER = \'|\');' % (schema, tableQuery, stageQuery)).collect()
...
if __name__ == '__main__':
    if(len(sys.argv) > 1):
        ...
        main(templateFilename = '../json/Templates/BasicQuestions_MasterTemplate.json', useLocalModel = False, encode = True, truncateLoad = False, loadSnowflake = True, runQueries = True, saveStageFiles=False, production=False)
